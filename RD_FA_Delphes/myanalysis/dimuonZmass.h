#ifndef dimuonZmass_h
#define dimuonZmass_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   static constexpr Int_t kMaxTrack = 51;
   static constexpr Int_t kMaxTower = 60;
   static constexpr Int_t kMaxEFlowTrack = 51;
   static constexpr Int_t kMaxEFlowPhoton = 36;
   static constexpr Int_t kMaxEFlowNeutralHadron = 11;
...
#ifdef dimuonZmass_cxx
dimuonZmass::dimuonZmass(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("samples/ee_ZH_Zmumu_Hbb_1k_CEPCdet.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("samples/ee_ZH_Zmumu_Hbb_1k_CEPCdet.root");
      }
...
dimuonZmass::~dimuonZmass()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t dimuonZmass::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t dimuonZmass::LoadTree(Long64_t entry)
{
...
void dimuonZmass::Init(TTree *tree)
{
...
void dimuonZmass::Show(Long64_t entry)
{
...
Int_t dimuonZmass::Cut(Long64_t entry)
{
