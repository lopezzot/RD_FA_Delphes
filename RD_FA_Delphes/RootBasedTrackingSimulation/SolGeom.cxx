...
void SolGeom::SolGeoFill()
{
	...
	if (fEnable[3])
	{
		...
		for (Int_t i = 0; i < NlDch; i++)
		{
			...
			fstLayU[fNlay] = pow(-1, i) * 2 * frPos[fNlay] *
				TMath::Sin(stAng / 2.) / fxMax[fNlay];	// Stereo angle (rad)  - Upper side
			fstLayL[fNlay] = 0;					// Stereo angle (rad) - 0(pi/2) = axial(z) layer - Lower side
...
void SolGeom::Draw()
{
	...
	Int_t il = 0; 
	Int_t ig = 0;
	for (Int_t i = 1; i < fBlay; i++)
	{
		if (lPos(i)>0.34 && lPos(i) < 2.02)		// Drift chamber layer
		{
			if (isMeasure(i))
			{
				char lab[10]; 
