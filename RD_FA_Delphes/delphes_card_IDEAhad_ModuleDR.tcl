...
module MomentumSmearing ChargedHadronMomentumSmearing {
	...
	(abs(eta) <= 3.0)                   * sqrt(0.0006^2 + pt^2*7.e-5^2)
    }
}
#	(abs(eta) <= 1.0)                   * sqrt(0.001^2 + pt^2*1.e-5^2) +
#	(abs(eta) > 1.0 && abs(eta) <= 3.0) * sqrt(0.01^2 + pt^2*1.e-4^2)

#       (abs(eta) <= 3.0)                   * ( ((-1.29796e-07)*pt^2+(4.42488e-05)*pt+(7.13683e-04))*(1-exp(-pt/(6.8343))+exp(-pt/(2.77524))) )
...
module DualReadoutCalorimeter Calorimeter {
  ...
  set SmearTowerCenter true
    ...
    #Barrel: deta=0.00195 towers up to |eta| <= 0.88 (45°)                                                                                        
    #Endcaps: deta=0.004 towers up to |eta| <= 3.0 (8.6° = 100 mrad)                                                                                           
    #Cell size: about 30cm x 30cm                                                  

    #barrel:                                                                                        
    set PhiBins {}
    for {set i -24} {$i <= 24} {incr i} {
        add PhiBins [expr {$i * $pi/24}]
    }
    ...
    for {set i 0} {$i <=20} {incr i} {
        set eta [expr {-3.00 + $i * 0.106}]
        add EtaPhiBins $eta $PhiBins
    }
    #same for 0.88 to 3.0                                                                           
    for  {set i 1} {$i <=20} {incr i} {
        set eta [expr {0.88 + $i * 0.106}]
        add EtaPhiBins $eta $PhiBins
    }
